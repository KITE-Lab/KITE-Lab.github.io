...
EXPECTED_REMOTE_URL="git@github.com:KITE-Lab/KITE-Lab.github.io.git"
...
Examples:
  ...
  ./scripts/git_submit_current.sh "refresh KITE Lab homepage"
...
if [[ -z "${COMMIT_MESSAGE}" ]]; then
  COMMIT_MESSAGE="chore: update KITE-Lab.github.io $(date '+%Y-%m-%d %H:%M:%S')"
fi
...
ORIGIN_URL="$(git remote get-url origin 2>/dev/null)" || die "remote 'origin' is not configured"

if [[ "${ORIGIN_URL}" != "${EXPECTED_REMOTE_URL}" ]]; then
  die "remote 'origin' is '${ORIGIN_URL}', expected '${EXPECTED_REMOTE_URL}'"
fi

if [[ "${UPSTREAM}" != origin/* ]]; then
  die "branch '${BRANCH}' must track an origin upstream; found '${UPSTREAM}'"
fi
